...
Se analizan los resultados obtenidos a partir de diferentes enfoques, desde \textit{baselines} simples hasta arquitecturas híbridas y neuronales, con el propósito de comparar su capacidad para modelar afinidades entre clientes y productos en el entorno B2B de consumo masivo.
...
\begin{table}[H]
    ...
    \begin{tabular}{l p{0.62\textwidth}}
        ...
        \midrule
        \textit{Precision@K} & Proporción de productos relevantes dentro del top-$K$ recomendado. \\
        \textit{Recall@K} & Porcentaje de productos comprados que aparecen en el top-$K$. \\
        \textit{MAP@K} & Promedio de la precisión acumulada, que pondera la posición de cada acierto en el ranking. \\
        \textit{NDCG@K} & Mide la calidad de la jerarquía del ranking, al asignar mayor peso a los aciertos en posiciones superiores. \\
        \textit{Diversity@K} & Disimilitud promedio entre los productos sugeridos dentro del top-$K$. \\
        \textit{Popularity Bias@K} & Grado de concentración del ranking en productos de alta frecuencia histórica. \\
        \bottomrule
    \end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{l c c}    
		\toprule
		\textbf{Modelo} & \textbf{\textit{Precision@10}} & \textbf{\textit{Recall@10}} \\
		\midrule
...
\begin{figure}[H]
    ...
    \caption[Comparación de Precision@10 frente a modelos \textit{baseline}]{Comparación global de \textit{Precision@10} entre los modelos avanzados y los modelos de referencia.}
...
\begin{figure}[H]
    ...
    \caption[Comparación de Recall@10 frente a modelos \textit{baseline}]{Comparación global de \textit{Recall@10} entre los modelos avanzados y los modelos de referencia.}
    \label{fig:recall10_comparativa}
\end{figure}


En términos de \textit{Precision@10}, los modelos basados en aprendizaje superan ampliamente a los enfoques no informados. Mientras que los \textit{baseline} alcanzan valores entre el 11\% y 17\%, los modelos ALS, \textit{LightFM} y las arquitecturas neuronales logran precisiones superiores al 29\%, con picos de 32{,}3\,\% en el modelo \textit{Two Towers} + ALS y 32{,}4\,\% en el modelo \textit{NeuCF}.

El comportamiento es consistente en \textit{Recall@10}. Los \textit{baseline} mantienen valores moderados (12--18\,\%), mientras que los modelos avanzados superan ampliamente este nivel. En particular, ALS alcanza un 33{,}7\,\%, seguido por \textit{Two Towers} + ALS con 34{,}5\,\% y \textit{NeuCF} con 34{,}3\,\%. Estos resultados confirman que las arquitecturas híbridas y neuronales permiten recuperar una mayor proporción de compras efectivas dentro del ranking recomendado.

Todos los enfoques evaluados superan de manera consistente a los modelos \textit{benchmark} en todas las métricas consideradas.
Esto confirma que incluso los modelos más simples basados en factores latentes ya capturan información estructural que los \textit{baseline} no pueden modelar, mientras que las arquitecturas híbridas y neuronales logran mejorar el desempeño al explotar patrones no lineales y relaciones implícitas en los datos.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c c c}
        \toprule
        \textbf{Modelo} &
        \textbf{\textit{Precision@10}} &
        \textbf{\textit{Recall@10}} &
        \textbf{\textit{MAP@10}} &
        \textbf{\textit{NDCG@10}} &
        \textbf{\textit{Diversity@10}} &
        \textbf{\textit{Popularity Bias@10}} \\
        \midrule
...
Los resultados muestran que las arquitecturas basadas en Spark (ALS) son significativamente más eficientes en entornos distribuidos, mientras que los modelos neuronales ofrecen mayor capacidad expresiva a costa de mayores recursos y tiempos de cómputo.  
...
\begin{tabular}{lcc}
\toprule
\textbf{Año-Mes} & \textbf{\textit{Precision@10}} & \textbf{\textit{Recall@10}} \\
\midrule
202506 & 29.9 \% & 34.1 \% \\
202507 & 30.1 \% & 33.5 \% \\
202508 & 31.5 \% & 33.7 \% \\
...
Al segmentar los resultados por unidad de negocio, se observa un comportamiento igualmente estable. La tabla~\ref{tab:robustez_hb} muestra que \texttt{CZA} y \texttt{NABS} mantienen niveles consistentes de \textit{precision} y \textit{recall} entre meses, mientras que \texttt{MATCH} presenta un \textit{recall} elevado y una precisión inferior. Lo importante es que estas diferencias permanecen prácticamente constantes en el tiempo, lo que demuestra que el modelo no amplifica variaciones estructurales del negocio.
